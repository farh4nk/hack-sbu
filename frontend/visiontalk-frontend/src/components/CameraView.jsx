...
const CameraView = () => {
  ...
  const analyzeFrame = async (imageData, userMode) => {
    try {
      ...
      if (data.objects) {
        setDetections(data.objects.map(([label, direction]) => ({
          label, direction, priority: ['car','truck','bus'].includes(label) ? 5 : ['person','dog'].includes(label) ? 3 : 1
        })));
      }
      return { narration: data.summary || '', detections: data.objects || [] };
    } catch (error) {
      setStatus('Demo mode');
      const demo = [
        { label: 'person', direction: 'center', priority: 5 },
        { label: 'chair', direction: 'left', priority: 2 }
      ];
      setDetections(demo);
      return { narration: userMode === 'live' ? 'person detected center' : 'Indoor space. Person ahead, chair left.', detections: demo };
    }
  };
  ...
  const startLiveMode = async () => {
    ...
    intervalRef.current = setInterval(async () => {
      ...
      if (result?.narration?.trim()) console.log('Speak:', result.narration);
    }, 2000);
  };
